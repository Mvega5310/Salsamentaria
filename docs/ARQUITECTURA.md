...
> Documento canónico y vivo de arquitectura. Consolida el `ARQUITECTURA.md` del último sprint (`docs/historial-sprints/11-pago-online-storefront-wompi/`); las 11 versiones anteriores quedan solo como archivo histórico de cómo fue creciendo. Actualízalo aquí, no en `historial-sprints/`.
...
> **Estado (2026-09-22):** capa de datos (18 migraciones) y app Next.js completas y reconstruidas en `backend/` y `frontend/` — ver secciones 1 a 12. `npm run build` y `npx tsc --noEmit` pasan limpios. Falta conectar credenciales reales (Supabase/Wompi/Factus) para operar en producción; ver `docs/PUESTA-EN-MARCHA.md`.
...
## 10. PWA del cliente (tienda pública)
...
**Pendiente:**
- [ ] Notificación al negocio (WhatsApp/push) cuando llega un pedido nuevo — ver 11.1 para el estado real (Realtime cerrado, WhatsApp pendiente de proveedor)
- [x] Pasarela de pago en el checkout del cliente final — cerrado con Wompi, ver 11.1
...
## 11. Notificación al negocio — pedidos en vivo

Cuando llega un pedido desde `/tienda/<slug>`, el tablero del negocio se
entera **sin refrescar**, vía Supabase Realtime (`postgres_changes` sobre
`orders`). No es una integración nueva — Realtime ya viene con Supabase,
así que no hubo que esperar a decidir un proveedor.

**Seguridad:** Realtime evalúa el mismo RLS que un `SELECT` normal — un
cajero solo recibe eventos de pedidos de *su* negocio, el aislamiento es el
mismo que ya está probado en el resto del sistema. Esto no lo pude probar
en vivo aquí (el sandbox solo tiene Postgres, no el servidor Realtime de
Supabase) — es el único punto de esta pieza que queda sin ejecutar, a
diferencia de casi todo lo demás en este documento.

Al llegar un pedido: sonido (generado con Web Audio, sin archivo de audio
que licenciar), aviso visual en pantalla, y notificación de escritorio si
el staff activó el permiso del navegador — útil cuando el tablero no está
en primer plano.

**Deliberadamente fuera de esto:** notificación por WhatsApp. Requiere
elegir un proveedor (Twilio, Meta Cloud API) — decisión de negocio pendiente,
mismo patrón que Factus/Wompi.

---

## 11.1 Pago en línea en el checkout del cliente (Wompi)

Se reutiliza el Wompi ya integrado para la suscripción — mismo cliente,
mismo patrón de webhook con firma verificada — pero es un flujo distinto:
pago **único** de un desconocido, vía **Checkout Web hospedado**
(formulario + redirección a una página de Wompi), no tokenización. El
servidor nunca ve datos de tarjeta en ningún momento del checkout público.

**Flujo:** el cliente arma el carrito → elige "Pagar en línea ahora" →
`create_storefront_order` crea el pedido con `payment_method='wompi_online'`
y `payment_status='pendiente'` → se le redirige a `/tienda/<slug>/pagar/<id>`,
que calcula la firma de integridad **en el servidor** (nunca en el
navegador — si se calculara ahí, cualquiera podría alterar el monto antes de
enviarlo) y renderiza el botón hospedado de Wompi → el webhook confirma el
pago real y actualiza `orders.payment_status` + inserta en `payments`.

**Detalle técnico real, no cosmético:** el atributo que exige Wompi
(`data-signature:integrity`) tiene dos puntos, algo que JSX no acepta como
nombre de prop. Se resuelve inyectando el HTML exacto que documenta Wompi
vía `dangerouslySetInnerHTML`, con todos los valores ya validados
server-side (el id del pedido se confirmó contra la base antes de llegar
ahí, nunca es texto libre de un usuario).

**Bug real encontrado y corregido al probar esto (migración 18):** el
índice único que soporta el `upsert` del webhook (tanto para pagos de
pedidos como, retroactivamente, para las facturas de plataforma de la
suscripción) era un índice **parcial** (`where provider_ref is not null`).
Postgres exige que `ON CONFLICT` repita exactamente esa condición para
poder usarlo — ni el `.upsert()` de Supabase-JS ni un `ON CONFLICT` plano
lo hacían, así que el webhook fallaría en producción la primera vez que
Wompi reenviara un evento (algo que hace normalmente). Se corrigió
volviendo el índice no-parcial: en SQL, `NULL` nunca choca consigo mismo,
así que no hacía falta la condición desde el principio. Probé el caso que
importa — Wompi reenviando el mismo evento dos veces — y confirmé que no
duplica el pago.

**Tablero:** un pedido con pago en línea pendiente muestra "Pago
pendiente" en rojo — el staff no debería empezar a preparar algo que
todavía no se ha cobrado. Una vez el webhook confirma, cambia a "Pagado en
línea".

---

## 12. Ejecución y despliegue (VS Code)

El proyecto se termina de ejecutar y desplegar fuera de esta conversación,
en VS Code. Para retomarlo ahí:

```
cd web
cp .env.example .env.local   # completar con las llaves reales
npm install
npm run dev                  # http://localhost:3000
```

Antes de correr la app, aplicar las migraciones `01` a `16` en el proyecto
Supabase (SQL Editor o `supabase db push`), en ese orden — cada una asume
que las anteriores ya corrieron.

Para desplegar en Vercel: conectar el repo, configurar todas las variables
de `.env.example` en el proyecto de Vercel (incluida `CRON_SECRET`, que
Vercel usa automáticamente para autenticar `vercel.json`), y registrar la
URL de producción de `/api/webhooks/wompi` en el dashboard de Wompi.
...
- [x] Tablero en vivo (Realtime) con alerta de sonido/escritorio en pedidos nuevos
- [x] Pasarela de pago en el checkout del cliente final (Wompi, reusando la integración de la suscripción)

**Siguiente**
- [ ] Notificación por WhatsApp (requiere elegir proveedor: Twilio o Meta Cloud API)
