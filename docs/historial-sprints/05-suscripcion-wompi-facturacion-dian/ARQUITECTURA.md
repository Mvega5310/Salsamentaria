...
- [x] Integración Factus (facturación DIAN desde el POS)

**Siguiente**
- [ ] PWA del cliente (catálogo público con marca por tenant + checkout)
- [ ] Cron de cobro mensual y UI de configuración DIAN por tenant (ver sección 9)
- [ ] Middleware de *gating* por plan (bloquear features fuera del plan contratado)
- [ ] Agente de impresora térmica (mostrador)
- [ ] Pantalla de pesaje con balanza conectada

---

## 11. Puntos que necesito confirmar

1. **Clasificación de IVA real** de los productos (con el contador) para reemplazar la semilla.
2. **Pasarela para el checkout de cada salsamentaría con sus propios clientes** (posible candidato: Bold) — decisión separada de Wompi, que aquí solo cobra la suscripción.
